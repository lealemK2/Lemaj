...
use rocket::response::status::{Accepted, Created, NoContent};
use crate::models::account::Account;
use rocket::serde::json::Json;
use crate::database::db::Db;
use rocket::State;

type Result<T, E = rocket::response::Debug<deadpool_postgres::PoolError>> = std::result::Result<T, E>;
...
#[get("/accounts/<id>")]
async fn get(db: &State<Db>, id: i32) -> Result<Accepted<Json<Account>>> {
    let account = accounts::get_account(db, id).await?;
    Ok(Accepted(Some(Json(account))))
}

#[get("/accounts?<page_number>&<page_size>")]
async fn get_all(db: &State<Db>, page_number: Option<i64>, page_size: Option<i64>) -> Result<Accepted<Json<Vec<Account>>>> {
    ...
    Ok(Accepted(Some(Json(accounts))))
}
